...
export class MessgaeService {
	...
	async createMessage(req: Request, dto: CreateMessageDto, conversationId: string) {
		...
		// checkout id 
		let receiverId
		(userId === conversation.creatorId) ? receiverId = conversation.friendId : receiverId = userId
	...
	async findMessage(req: Request, query: FindMessageDto, conversationId: string) {
		...
		const nextCursor = messages.length > 0 ? resultMessages[messages.length - 1].id.toString() : null
	...
	async loadAllMessage(req: Request, query: LoadAllMessageDto, conversationId: string) {
		...
		const nextCursor = resultMessage.length > 0 ? resultMessage[resultMessage.length - 1].id.toString() : null
