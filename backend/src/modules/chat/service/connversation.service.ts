...
export class ConversationService {
	...
	async createConversation(req: Request, friendId: string) {
		...
		// check available 
		const conversation = await this.prismaService.conversation.findUnique({
			where: { creatorId_friendId: { creatorId: creatorId, friendId: friendId } }
		})
	...
	async loadingConversation(req: Request, query: LoadAllConversationDto) {
		...
		const nextCursor = conversations.length > 0 ? resultConversations[conversations.length - 1].id : null
